...
4. The FastAPI Cloudflare Worker lives in `backend/`: `app/worker.py` is the ASGI entrypoint, `wrangler.jsonc` declares its D1 binding, and `migrations/` owns the D1 schema. Use the Python Workers `pywrangler` CLI from this directory for local Workers-runtime testing. See [Python Workers documentation](https://developers.cloudflare.com/workers/languages/python/).
